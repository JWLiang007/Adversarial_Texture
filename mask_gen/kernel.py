...
def get_kernel(size,device):
    kernel = torch.ones(size).unsqueeze(0).unsqueeze(0).to(device)
    kernel[:,:,size[0]//2,size[0]//2] = 0
    kernel_pad = F.pad(kernel, [size[0]//2] * 4)
    kernel = F.conv2d(kernel_pad,kernel,bias=None, stride=1)
    kernel[:,:,size[0]//2,size[0]//2] = 0
    return kernel