...
def preprocess(device):
    ...
    return model, loader,tps, results_dir
...
def train_EGA(device):
    model, loader,tps, results_dir = preprocess(device)
    ...
    gen, model, iter_target,optimizerG ,optimizerD,optimizerD_ctr= accelerator.prepare(
        gen, model, iter_target,optimizerG,optimizerD,optimizerD_ctr)
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        for i_batch, data in tqdm(enumerate(iter_target), desc=f'Running epoch {epoch}',
            ...
            if  pargs.skip_load_img :
                ...
                images[:batch_size,:,s_r:t_r,s_c:t_c] = images[:batch_size,:,s_r:t_r,s_c:t_c] +adv_patch_tps* fit_coef * 0.05     
                ...
                images[:_batch_size,:,s_r:t_r,s_c:t_c] = images[:_batch_size,:,s_r:t_r,s_c:t_c] +adv_patch_tps* fit_coef * 0.05
                images = torch.clip(images,0,1)
            ...
            disc, pj, pm = [det_loss.new_zeros(1)]*3
            tv_loss = tv * args.tv_loss
